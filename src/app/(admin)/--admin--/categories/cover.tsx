...
const Cover = ({
   ...
   const dialogImportedFetchprojectsImages = async () => {
      const projectsImages = await getprojectsImages()
      setprojectsImages(projectsImages)
   }

   const getprojectsImages = async () => {
      const res = await fetch(`/api/--admin--/project?id=${_id}`)
      ...
      projects?.length
         ? projects.reverse().map((project) => {
              projectsImages.push(project.frontSrc)
              project.backSrc && projectsImages.push(project.backSrc)
              project.gallery.length &&
                 project.gallery.forEach((image: string) => {
         ...
         <div
            ...
            onClick={() => {
               if (!importDialog.current) {
                  dialogImportedFetchprojectsImages()
         ...
         >
            {cover ? (
               <Image
                  ...
                  src={`https://tabrizian.storage.iran.liara.space/tabrizian_codes/projects/${cover}`}
                  alt={_id}
                  height={50}
                  width={50}
               />
                        ...
                        <div className='relative w-full items-center rounded-xl px-3 py-8 shadow-[0_6px_10px_0_#00000040]'>
                           ...
                           <div className='rounded-xl'>
                              <Image
                                 ...
                                 src={`https://tabrizian.storage.iran.liara.space/tabrizian_codes/projects/${preview}`}
                                 fill
                                 alt={name}
