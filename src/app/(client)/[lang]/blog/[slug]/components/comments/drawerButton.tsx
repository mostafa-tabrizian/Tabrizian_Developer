   ...
   return (
      <>
         <button
            ...
            className='flex items-center gap-1 border-r-2 border-slate-700 pr-4 text-slate-700'
         >
            <span className='yekan text-base'>{comments.length.toLocaleString(lang)}</span>
            <svg
         ...
         {commentImport ? (
            <DrawerComments panel={panel} setPanel={setPanel} blogId={blogId} userIP={userIP} comments={JSON.parse(JSON.stringify(comments))} />
         ) : (
            ''
         )}
      </>
